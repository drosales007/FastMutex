...
public class UILockTester {
	...
	try {
            ...
            myId = comm.getMyId();
            FastMutex.srvLinkers.put(myId, (UILinker) comm);
    ...
    void runAutomation(){
        ...
        String msg;
        String suffix;
        int iters = FastMutex.iterations;
        for (int i = 0; i < iters; i++) {
            ...
            FastMutex.csLabel.addServer(myId);
            FastMutex.mqueue.enqueue(msg);
            System.out.println(msg);
            int csDelay = FastMutex.srvLinkers.get(myId).csDelay;
            // Stay in the CS for the required time
            Util.mySleep(csDelay * 1000);
            System.out.println("Server " + myId + " in CS for "
                               + csDelay + " seconds");
            t = System.currentTimeMillis();
            FastMutex.csLabel.removeServer(myId);
    ...
    void runManual(){
        ...
        while(true){
            ...
            if (FastMutex.srvLinkers.get(myId).req){
                FastMutex.srvLinkers.get(myId).req = false;
                lock.requestCS();
                long t = System.currentTimeMillis();
            ...
            if (FastMutex.srvLinkers.get(myId).rel){
                FastMutex.srvLinkers.get(myId).rel = false;
                long t = System.currentTimeMillis();
                FastMutex.csLabel.removeServer(myId);
                msg = "Server " + myId + " left the CS at " 
                      + millisToTime(t);
