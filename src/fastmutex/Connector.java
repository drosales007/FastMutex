...
/**
 *
 * Sets the servers neighbors and creates connections between servers
 * 
 */
public class Connector {
	...
	for (Integer pid : neighbors) {
            if (pid > myId) {
		InetSocketAddress addr = myNameclient.searchName(basename + pid,
                                                                 true);
    ...
    int getLocalPort(int id) {
        return Symbols.ServerPort + 20 + id;
    }
